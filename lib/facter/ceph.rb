...
Facter.add(:ceph_client_glance) do
  confine { File.exist? '/etc/ceph/ceph.client.admin.keyring' }
  setcode do
    client_keyring = `/usr/bin/ceph auth get client.glance` if File.executable?('/usr/bin/ceph')
...
Facter.add(:ceph_client_cinder) do
  confine { File.exist? '/etc/ceph/ceph.client.admin.keyring' }
  setcode do
    client_keyring = `/usr/bin/ceph auth get client.cinder` if File.executable?('/usr/bin/ceph')
...
Facter.add(:ceph_client_cinder_backup) do
  confine { File.exist? '/etc/ceph/ceph.client.admin.keyring' }
  setcode do
    client_keyring = `/usr/bin/ceph auth get client.cinder-backup` if File.executable?('/usr/bin/ceph')
