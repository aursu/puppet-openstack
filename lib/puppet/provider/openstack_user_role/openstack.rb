...
Puppet::Type.type(:openstack_user_role).provide(:openstack, parent: Puppet::Provider::Openstack) do
  ...
  def self.instances
    ...
    user_instances = Puppet::Type.type(:openstack_user).instances.map { |u| [u[:id], u[:name]] }.to_h
    role_instances = Puppet::Type.type(:openstack_role).instances.map { |r| [r[:id], r[:name]] }.to_h

    user_role_list = {}

    provider_list.map do |entity|
      user_id = entity['user']
      role_id = entity['role']
